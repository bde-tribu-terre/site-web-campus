...
// Crédits à l'association et à moi
    let version = await fetch('/version.txt').then(response => response.text().then(text => {return text}));

    let TT = L.control({position: 'bottomright'});
    TT.onAdd = function () {
        let div = L.DomUtil.create('div', 'leaflet-control-attribution leaflet-control');
        div.innerHTML += 'Anaël BARODINE, <a href="https://bde-tribu-terre.fr">Tribu-Terre</a>, v' + version + '<!-- Un plaisir ;) -->';
        return div;
    };
    TT.addTo(carte);
